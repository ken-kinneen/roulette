  ...
  return (
    <div className="game-over-screen">
      <div className="game-over-content">
        {isPvP ? (
          // PvP Win/Lose Display
          <>
            <div className={`pvp-result-icon ${playerWon ? 'victory' : 'defeat'}`}>
              {playerWon ? '🏆' : '💀'}
            </div>
        ...
        {!isPvP && globalLeaderboard.length > 0 && (
        ...
        <p className="epitaph">
          {isPvP 
            ? (playerWon ? '"Victory is sweetest when earned through courage."' : '"Even the bravest fall to fate."')
            : '"Fortune favors the bold, but the revolver favors no one."'}
        </p>

        <button className="play-again-button" onClick={resetGame}>
          <span>{isPvP ? 'BACK TO MENU' : 'TRY AGAIN'}</span>
        </button>
      </div>
    </div>
  );
